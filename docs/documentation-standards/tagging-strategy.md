...
description: "Controlled vocabulary for document classification in logmask-python-library"
...
Controlled tag vocabulary for the logmask-python-library repository. Consistent tagging enables human navigation and RAG system retrieval.
...
|-----|---------|----------|
| `anonymization` | Replacement engine, automaton, map merging, fake value generation | Core anonymization pipeline, not detection |
| `detection` | Parsers, pattern matching, identifier discovery, scanner | Finding identifiers in text, not replacing them |
| `cli` | Command-line interface, argparse handlers, user-facing commands | CLI layer, not underlying engine |
| `maps` | CSV map format, scope merging, global/project maps, map CRUD | Map data management, not the replacement that consumes maps |
| `testing` | Unit tests, coverage, fixtures, roundtrip validation | Test infrastructure and results |
| `security` | Operational security context, MSP use cases, data leakage prevention | The "why" of the tool, not the "how" |
...
| `reference` | Lookup information: identifier types, map format, CLI usage |
| `specification` | Build spec, design specs |
| `report` | Test coverage reports, validation results |
...
| `aho-corasick` | pyahocorasick automaton |
